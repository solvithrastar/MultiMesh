...
class Exodus(object):
    ...
    def attach_field(self, name, values):
        ...
        assert self.mode in ["a"], (
            "Attach field option only " "available in mode 'a'"
        )

        with exodus(self._filename, self.mode) as e:
            if values.size == self.nelem:
                e.put_element_variable_values(
                    blockId=1, name=name, step=1, values=values
                )
                ...
                raise ValueError(
                    "Shape matches neither the nodes nor the " "elements"
                )
    ...
    def get_element_field(self, name):
        ...
        assert self.mode in ["r", "a"], (
            "Attach field option only " "available in mode 'r' or 'a'"
        )
        assert name in self.elem_var_names, (
            "Could not find " "the requested field"
        )
        with exodus(self._filename, self.mode) as e:
            values = e.get_element_variable_values(
                blockId=1, name=name, step=1
            )
        return values
    ...
    def get_nodal_field(self, name):
        ...
        with exodus(self._filename, self.mode) as e:
            assert (
                name in e.get_node_variable_names()
            ), "Could not find the requested field"
